...
class PlaylistController extends Controller {
	...
	private function writePlaylistToDB($sName){
		...
		if ($this->db->insertIfNotExist('*PREFIX*mp3_player_playlists', ['user_id' => $this->userId, 'name' => $sName])) {
					
			$insertid = $this->db->getInsertId('*PREFIX*mp3_player_playlists');
			...
			$stmt = $this->db->prepareQuery( 'SELECT `id` FROM `*PREFIX*mp3_player_playlists` WHERE `user_id` = ? AND `name` = ?' );
	...
	private function updatePlaylistToDB($id,$sName){
		$stmt = $this->db->prepareQuery( 'UPDATE `*PREFIX*mp3_player_playlists` SET `name` = ? WHERE `user_id`= ? AND `id`= ?' );
	...
	private function getPlaylistsforUser(){
		$SQL="SELECT  `id`,`name` FROM `*PREFIX*mp3_player_playlists`
			 			WHERE  `user_id` = ?
			 			ORDER BY `name` ASC
			 			";
	...
	private function getSongIdsForPlaylist($iPlaylistId){
		$SQL="SELECT  `track_id` FROM `*PREFIX*mp3_player_playlist_tracks`
			 			WHERE  `playlist_id` = ?
	...
	public function addTrackToPlaylist(){
		...
		try {
			$this->db->insertIfNotExist('*PREFIX*mp3_player_playlist_tracks',
				array(
	...
	public function sortPlaylist(){
		...
		foreach($iTrackIds as $trackId){
			$stmt = $this->db->prepareQuery( 'UPDATE `*PREFIX*mp3_player_playlist_tracks` SET `sortorder` = ? WHERE `playlist_id` = ? AND `track_id` = ?' );
	...
	public function removeTrackFromPlaylist(){
		...
		try {
			$sql = 'DELETE FROM `*PREFIX*mp3_player_playlist_tracks` '
	...
	public function removePlaylist(){
		 ...
		 try {
			$sql = 'DELETE FROM `*PREFIX*mp3_player_playlists` '
	...
	*/
	public static function post_deleteUser($arguments) {
		/*
		$stmt = \OCP\DB::prepare('SELECT `id` FROM `*PREFIX*mp3_player_playlists` '
				. 'WHERE `user_id` = ?');
