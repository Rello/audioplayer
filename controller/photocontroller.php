...
class PhotoController extends Controller {
	...
	public function cropPhoto(){
		...
		$response = new TemplateResponse('mp3_player', 'part.cropphoto', $params, '');
	...
	public function saveCropPhoto(){
		...
		//\OCP\Util::writeLog('mp3_player','MIMI'.$tmpkey,\OCP\Util::DEBUG);	
		$data = \OC::$server->getCache()->get($tmpkey);
		if($data) {
...
/**
	...
	public function uploadPhoto(){
		...
		if($error !== UPLOAD_ERR_OK) {
			...
			\OCP\Util::writeLog('mp3_player','Uploaderror: '.$errors[$error],\OCP\Util::DEBUG);	
		}

		if(file_exists($file['tmp_name'])) {
			$tmpkey = 'mp3_player-photo-'.md5(basename($file['tmp_name']));
			...
			if($image->loadFromFile($file['tmp_name'])) {
				...
				if(!$image->fixOrientation()) { // No fatal error so we don't bail out.
					\OCP\Util::writeLog('mp3_player','Couldn\'t save correct image orientation: '.$tmpkey,\OCP\Util::DEBUG);
				}
