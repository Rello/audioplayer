...
Audios.prototype.buildSongContainer = function (eventTarget) {
    ...
    divSongContainer.css({'top': scrollTop + activeAlbum.offset().top + iTop});
    appContent.animate({'scrollTop': scrollTop + activeAlbum.offset().top - iScroll}, iAnimateTime, 'linear', function () {
        ...
        success: function (jsondata) {
            ...
            if (jsondata.status === 'success') {
                ...
                $(jsondata.data).each(function (i, el) {
                    ...
                    var fav_action;
                    if (el.fav === 't') {
                        fav_action = $('<i/>').addClass('icon icon-starred').css({'opacity': 0.3}).attr({'data-fileid': el.fid}).click($this.favoriteUpdate.bind($this));
                    } else {
                        fav_action = $('<i/>').addClass('icon icon-star').attr({'data-fileid': el.fid}).click($this.favoriteUpdate.bind($this));
                    }
                    var stream_type;
                    var streamUrl;
                    var spanAction;
                    ...
                    if (el.mim === 'audio/mpegurl' || el.mim === 'audio/x-scpls' || el.mim === 'application/xspf+xml') {
                        ...
                        spanAction = $('<span/>').addClass('actionsSong').append(fav_action).append($('<i/>').addClass('ioc ioc-volume-off'));
                    } else {
                        stream_type = false;
...
Audios.prototype.favoriteUpdate = function (evt) {
    ...
    $.ajax({
        ...
        data: {'fileId': fileId, 'isFavorite': isFavorite}
    });
    return false;
};
...
Audios.prototype.renamePlaylist = function (evt) {
    ...
    if ($('.plclone').length === 1) {
        ...
        myClone.find('input[name="playlist"]').bind('keydown', function (event) {
            if (event.which === 13) {
...
Audios.prototype.checkNewTracks = function () {
    $.ajax({
        type: 'POST', url: OC.generateUrl('apps/audioplayer/checknewtracks'), success: function (data) {
            if (data === 'true') {
