...
OCA.Audioplayer.Core = {
    ...
    selectCategoryItemFromPreset: function () {
        if (OCA.Audioplayer.Core.CategorySelectors[1] && OCA.Audioplayer.Core.CategorySelectors[1] !== 'undefined') {
...
OCA.Audioplayer.Category = {
    ...
    loadIndividualCategory: function (evt, callback) {
        ...
        OCA.Audioplayer.Core.AjaxCallStatus = $.ajax({
            ...
            success: OCA.Audioplayer.Category.onGetCategoryItemsResponse.bind(this, callback, category, categoryActive, PlaylistId)
        });
    },

    onGetCategoryItemsResponse: function (callback, category, categoryActive, playlistId, jsondata) {
...
OCA.Audioplayer.UI = {
    ...
    trackClickHandler: function (callback) {
        ...
        playlist.each(function (index, elm) {
            ...
            element.find('.title').on('click',
                OCA.Audioplayer.UI.onTitleClick.bind(OCA.Audioplayer.UI, getcoverUrl, can_play, playlist, element)
            );
        });
    ...
    onTitleClick: function (coverUrl, can_play, playlist, element) {
    ...
    showInitScreen: function (mode) {
        ...
        if (mode === 'smart') {
            ...
            OCA.Audioplayer.UI.EmptyContainer.html('<span class="no-songs-found">' + t('audioplayer', 'Welcome to') + ' ' + t('audioplayer', 'Audio Player') + '</span>');
            OCA.Audioplayer.UI.EmptyContainer.append('<span class="no-songs-found-pl"><i class="ioc ioc-refresh" title="' + t('audioplayer', 'Scan for new audio files') + '" id="scanAudiosFirst"></i> ' + t('audioplayer', 'Add new tracks to library') + '</span>');
            OCA.Audioplayer.UI.EmptyContainer.append('<a class="no-songs-found-pl" href="https://github.com/rello/audioplayer/wiki" target="_blank">' + t('audioplayer', 'Help') + '</a>');
        }
    },
...
OCA.Audioplayer.Backend = {
    favoriteUpdate: function (evt) {
        ...
        if (OCA.Audioplayer.Core.CategorySelectors[1].toString().substring(0, 1) === 'S') {
            return;
        }
