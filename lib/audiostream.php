...
class AudioStream {
	...
	private function setHeader() {
		...
		if (isset($_SERVER['HTTP_RANGE'])) {
			...
			header("Content-Range: bytes ".$this->iStart."-".$this->iEnd."/".$this->iSize);
			//\OCP\Util::writeLog('mp3_player','SEQ:'.$this->iStart."-".$this->iEnd."/".$this->iSize.'length:'.$length,\OCP\Util::DEBUG);
		} else {
		...
		if($this->isStream){
	        ...
	        while(!feof($this->stream) && $curPos <= $this->iEnd) {
	           if( connection_aborted() || connection_status() !== 0 ) {
	           	\OCP\Util::writeLog('mp3_player','Connection aborted',\OCP\Util::DEBUG);
				   $this->closeStream();
			  	}
