...
use OCA\audioplayer\Http\AudioStreamResponse;
...
class MusicService
{
    ...
    public function createPublicAudioStream(string $token, string $file): AudioStreamResponse
    {
        $share = $this->shareManager->getShareByToken($token);
        $fileOwner = $share->getShareOwner();
        ...
        return new AudioStreamResponse($filenameAudio, $fileOwner);
    }

    public function createAudioStream(?string $file, ?string $t): AudioStreamResponse
    {
        if ($t) {
        ...
        return new AudioStreamResponse($filename, $this->userId);
    }
}
